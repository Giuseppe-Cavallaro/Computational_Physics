 ...
 *   File check3.c
 *
 *   Calculate <m^2> with and without accept/reject 
 *   with: should be independent of dtau
 *   without: should go like O(dtau^2)
 */ 
...
double hmc_check3(act_params_t *apars, hmc_params_t *pars, FILE *f, int accept_reject_step) {
	...
	for (n = 0; n < ntraj; n++) {
		...
		if ((n+1) % naccu == 0) {
			avgM2 /= naccu;
			if (f != NULL) fprintf(f, "%e,%e,%e\n", dtau, dtau*dtau, avgM2);
			avgM2 = 0.0;
		}
	}
	
	if (accept_reject_step == 1) return (double)nAcc / (double)nProp;
	else return 1.0;
}
...
int main(int argc, char* argv[]) {
	...
	printf("WITHOUT accept/reject: \n");
	fprintf(f, "dtau,dtau2,m2\n");
    for (i = max_nstep; i >= min_nstep; i -= d_nstep) {
		ranlxd(phi, V);
		hmc_params.nstep = i;
		acc = hmc_check3(&act_params, &hmc_params, f, 0);
		printf("%03i / %i - acc = %f\n", i, max_nstep, acc);
	}
	fprintf(f, "\n");
	...
	for (i = max_nstep; i >= min_nstep; i -= d_nstep) {
		ranlxd(phi, V);
		hmc_params.nstep = i;
		acc = hmc_check3(&act_params, &hmc_params, f, 1);
		printf("%03i / %i - acc = %f\n", i, max_nstep, acc);
	}
    fclose(f);
